...
# Part 2
def part2(inpt: str):
    nums = sorted(int(n) for n in inpt.rstrip().split(","))  # sort to make better use of memoization
    low, high = min(nums), max(nums)
    return sorted(sum(get_exponential_fuel_cost(abs(m - n)) for n in nums) for m in range(low, high))[0]
